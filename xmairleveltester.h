...
public:
     ...
     */
    void run_tests(std::shared_ptr<lo::Address> mixer_addr, uint num_steps, bool log);
     ...
     */
      ...
      * @return Returns a shared pointer to a lo:Address or nullptr if none is found.
      */
     std::shared_ptr<lo::Address> find_mixer(); // Search for a mixer console
      ...
      */
     int check_fader_level(std::shared_ptr<lo::Address> mixer_addr, float flevel, bool log = true); // Test a level

     /**
      * @brief Count distinct Node fader values
      * @param mixer_addr Address of mixer to run the test on.
      */
     int count_node_db(std::shared_ptr<lo::Address> mixer_addr);
      ...
      */
     void set_fader_float(std::shared_ptr<lo::Address> mixer_addr, float flevel);

     /**
      * @brief Query the current float representation of the fader.
      * @return Current fader level (float) or -1.f in case of error.
      */
     float query_fader_float(std::shared_ptr<lo::Address> mixer_addr);
      ...
      * @return Current fader level (float) or -1.f in case of error
      */
     void set_fader_db(std::shared_ptr<lo::Address> mixer_addr, std::string db);

     /**
      * @brief Query the current dB string representation of the fader.
      * @return Current fader level (dB) as string
      */
     std::string query_fader_db(std::shared_ptr<lo::Address> mixer_addr);

private:
    uint _num_steps; // Number of steps to test
    uint _step = 0;
    uint _channel; // channel number to use for testing
    std::promise<std::shared_ptr<lo::Address> > _promise_mixer_addr;
